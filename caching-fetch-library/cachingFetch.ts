...
export type UseCachingFetchResponse = {
    isLoading: boolean;
    data: unknown | null;
    error: Error | null;
};
...
export const useCachingFetch: UseCachingFetch = (url) => {
    ...
    useEffect(() => {
        const init = async () => {
            try {
                ...
                const dataJson = await blob.text();
    ...
    //     data: null,
    //     isLoading: false,
    //     error: new Error('UseCachingFetch has not been implemented, please read the instructions in DevTask.md'),
    // };
};
